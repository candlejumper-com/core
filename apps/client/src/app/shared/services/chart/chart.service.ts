...
export type Chart = ViewedChart & { candles: ICandle[]; orders: IOrder[]; indicators: any[] }
...
export class ChartService {
  ...
  init() {
    ...
    this.chartsInTabs$.next(this.profileService.profile.settings.client.main.charts.filter(Boolean))
  ...
  showChart(chartId: ChartId) {
    const chart = this.chartsInTabs$.value.find((chart) => chart.id === chartId)
    if (!chart) {
  ...
  getChartData(id: ChartId, symbol: ISymbol, type: ChartType, snapshot?: any): ViewedChart {
    return {
      id,
      symbol,
      type,
  ...
  rememberChartsInTabs() {
    this.profileService.profile.settings.client.main.charts = this.chartsInTabs$.value.filter(
      (charts) => charts.type === 'MAIN',
    )
  }
  ...
  close(chartId: ChartId) {
    ...
    const index = this.chartsInTabs$.value.findIndex((chart) => chart.id === chartId)
